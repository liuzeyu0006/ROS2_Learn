...
class TurtleController : public rclcpp::Node // 定义了一个名为 TurtleController 的类，它继承自 rclcpp::Node。
{
public:
    TurtleController() : Node("turtle_controller")
    {
        ...
        patrol_server_ = this->create_service<Patrol>( // 创建一个服务，用于处理巡逻请求。
            "patrol",
        ...
        velocity_publisher_ = this->create_publisher<geometry_msgs::msg::Twist>( // 创建一个发布者，用于发布速度指令。
            "/turtle1/cmd_vel", 10);
        pose_subscription_ = this->create_subscription<turtlesim::msg::Pose>( // 创建一个订阅者，用于接收海龟的位置信息。
            "/turtle1/pose", 10,
...
private:
    void on_pose_received_(const turtlesim::msg::Pose::SharedPtr pose) // on_pose_received_ 是一个回调函数，当接收到海龟的位置信息时被调用
    {
        ...
        if (distance > 0.1)
        {
            if (fabs(angle) > 0.2)
            {
                message.angular.z = angle; // 修正角速度方向
            }
            else
            {
                message.linear.x = k_ * distance; // 添加线速度设置
            }
        }
        else
        {
            message.linear.x = 0; // 到达目标附近停止
        }
...
int main(int argc, char *argv[])
{
    ...
    auto node = std::make_shared<TurtleController>(); // 修正类名
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}